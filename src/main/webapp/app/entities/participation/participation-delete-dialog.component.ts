...
@Component({
  selector: 'swg-participation-delete-dialog',
...
@Component({
  selector: 'swg-participation-delete-popup',
  template: ''
})
export class ParticipationDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ participation }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(ParticipationDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
